...
const ReceivedRequests = ({navigation}) => {
  ...
  const getAllCompanies = async () => {
    setLoading(true);
    try {
      const res = await getData(`/form/receive/requests/${userDetails?._id}`);
      console.log(res);
  ...
  return (
    <SafeAreaView style={{flex: 1}}>
      <Topbar navigation={navigation} title="Received requests" />
      <ScrollView
        style={{
          flex: 1,
            ...
            {requests.length === 0 ? (
              ...
              <>
                {requests?.map((data, idx) => (
                  <RequestCard key={idx} data={data} navigation={navigation} />
                ))}
              </>
            )}
          </>
